...
public:
    ...
    void subscribe(IMessageListener *inListener, char *mqttSubTopicIn) {
        ...
        mqttClient.setCallback([inListener](char *topic, uint8_t *payload, unsigned int length) {
            inListener->onMessage(topic, payload, length);
        });

        Serial.print("Subscribe to: ");
        Serial.println(this->mqttSubTopic);
        mqttClient.subscribe(this->mqttSubTopic); // todo handle error
    }

    boolean publish(char *baseTopic,
                    char controllerId[18],
                    char *sensorId,
                    float humidity,
                    float temperature,
                    float voltage,
